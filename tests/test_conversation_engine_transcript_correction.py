...
def test_korekta_transkrypcji_i_shutdown_intencja_obsluguja_jarwis_wylacz_sie(tmp_path):
    from src.voice_commands import is_shutdown_command
    ...
    text = engine.correct_transcript("jarwis wylacz sie", allow_llm=False)

    assert text == "jarvis wyłącz się"
    assert is_shutdown_command(text) is True
